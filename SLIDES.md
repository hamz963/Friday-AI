...
* **Core Promise**: Tell NOVA what you want. NOVA determines how to accomplish it, executes the work safely, verifies the result, and stays with the mission until the objective is completed.
...
### NOVA OS vs. OpenJarvis vs. OpenClaw

| Feature | **NOVA OS (This Project)** | **OpenJarvis** | **OpenClaw** |
|:---|:---|:---|:---|
...
```
NOVA CORE ENGINE
 ├── NOVA-core       (Config, Telemetry & Hardware Profiler)
 ├── NOVA-llm        (Multi-Provider Adapters & Web Scraper)
 ├── NOVA-memory     (SQLite Agent History & Embeddings)
 ├── NOVA-refiner    (Whisper Flow Speech Filler Filter)
 ├── NOVA-terminal   (Sandboxed Command Execution Kernel)
 ├── NOVA-git        (Repository Working Tree Operator)
 ├── NOVA-diagnostics(Compiler Auto-Debugging Loop)
 ├── NOVA-api        (Axum REST API & Embedded Web Server)
 ├── NOVA-desktop    (System Keyboard & Mouse Drivers)
 ├── NOVA-browser    (Headless Chrome Automation Controller)
 └── NOVA-agents     (Multi-Agent Workflow Orchestrator)
```
...
NOVA acts as the master orchestrator, dynamically delegating subtasks across specialized domain agents:
...
2. **`NOVA-terminal` Command Sandbox**: Regex-based filtering engine that detects and blocks destructive operations (e.g. `rm -rf`, raw privilege escalations).
...
* **Windows**: Native **`nova-installer.msi`** (compiled via WiX Toolset) & standalone `nova.exe` (11.9 MB).
* **macOS**: Native Apple Silicon/Intel **`nova-macos-x64.tar.gz`**.
* **Linux**: Native **`nova-linux-x64.tar.gz`** & `.deb` packages.
...
* **Persistent Memory**: SQLite database (`nova_memory.db`) preserves historic chat context across reboots.

---

<!-- slide -->

## Slide 11: Competitive Edge: NOVA OS vs. Odysseus.ai

| Feature | 🤖 **NOVA OS** | 🧠 **Odysseus.ai** (PewDiePie) | 💻 **OpenInterpreter** |
|:---|:---|:---|:---|
...
| **Safety Engine** | **Regex Sandbox (`NOVA-terminal`)** | Unrestricted / Basic shell | Confirmation Prompts |
...
👉 **Live Demo**: [hamz963.github.io/nova-ai-assistant](https://hamz963.github.io/nova-ai-assistant/)  
📦 **GitHub Releases**: [github.com/hamz963/nova-ai-assistant](https://github.com/hamz963/nova-ai-assistant)
